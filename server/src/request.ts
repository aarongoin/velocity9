...
export class RequestData extends Readable {
  _source: OnData;
...
export class AppRequest<Data = unknown, Context extends Record<string, any> = {}> implements HttpRequest<Data, Context> {
  ...
  arrayBuffer(valid = true): Promise<ArrayBuffer> {
    ...
    if (valid && this.data) return Promise.resolve(this.data as unknown as ArrayBuffer);
  ...
  text(valid = true): Promise<string> {
    // short-circuit if we've already parsed the data
    if (valid && this.data) return Promise.resolve(this.data as unknown as string);
